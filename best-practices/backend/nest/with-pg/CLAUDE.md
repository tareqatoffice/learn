...
| `npm run test:e2e` | E2E tests (Testcontainers PostgreSQL — requires Docker) |
| `npm run migration:run` | Apply TypeORM migrations |
| `npm run migration:generate -- src/database/migrations/MigrationName` | Generate a migration from entity changes |
...
| Controllers | HTTP concerns only. No business logic. Always type return values. UUID `id` params use the built-in `ParseUUIDPipe` and `@IsUUID()` on DTOs. |
...
| Database | TypeORM Repository pattern. `@InjectRepository`. Migrations only — never `synchronize: true` in production. Query Builder for complex queries. |
