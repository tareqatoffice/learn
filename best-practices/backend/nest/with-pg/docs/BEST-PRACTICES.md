...
- Connect via `TypeOrmModule.forRootAsync()` so `ConfigService` provides all credentials — never hardcode connection strings.
...
- Test services in isolation. Mock TypeORM repositories using `getRepositoryToken(Entity)`.
...
- Add indexes on all fields used in queries, sorts, or unique constraints (see Entity Definition section).
- Paginate all list endpoints. Never return an unbounded list from the database.
- Use `select` to fetch only required columns on list endpoints — avoid loading large or sensitive fields unnecessarily:
...
- Use `@ApiResponse()` on controller methods to document possible status codes. The runtime return type is the entity (serialized by `ClassSerializerInterceptor`); `type: UserResponseDto` documents the wire shape:
...
> The frontend mirrors `Paginated<T>` — or, better, generates its types from this API's OpenAPI spec.
